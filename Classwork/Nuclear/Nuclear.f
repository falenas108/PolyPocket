...
C ***********************************************
      INTEGER FUNCTION INROD(ENERG,R,OLDXY,NEWXY,ANGL,SIGN,LOCTH)
      REAL, DIMENSION(36)  :: LOCTH
      REAL NEWXY
...
C *** The WRITE(5,*) is for multiplying the cross-section by the velocity.
      INROD=0
      DO L=1,3
        IF(ABS(NEWXY).Lt.(.09)) THEN
            WRITE(5,*)ENERG,R
            ...
            ELSE
                INROD=4
            END IF
C *** This finds if the neutron will hit the radius R,2R, or 3R while it travels the next distance.
        ELSE IF(((OLDXY*SIGN).lt.(L*R*SIGN)).and.((L*R*SIGN).lt.(NEWXY*SIGN))) THEN
            IF((R-XY)/DISTXY>0) THEN
                DO J=1,36
C *** This part is not exact, but is a good approximation.  It tests if the neutron is in a transmitted rod.
                    IF ((ABS(ANGL-LOCTH(J)).Lt.(1.5*.1/(6.28*R))).or.((ABS(ANGL-LOCTH(J)-6.28)).lt.(1.5*.1/(6.28*R)))) THEN
                         WRITE(5,*)ENERG,R
                        IF((ENERG).Lt.(.0005)) THEN
                            INROD=1
                            EXIT
...
C *******************************************************
      ...
      SUBROUTINE INITIAL(ENERG,PHI,ANGL,EX,EY,EZ,R,X,Y,Z)
...
C ************************************************
      PROGRAM NUCLEAR
C *** Using spherical coordinates, LOCTH is the angle rods are at.  R is the radius, RUNS is the number of runs
C *** each radius goes through, NEWXY and OLDXY are used to see if the neutron passed through a rod.
      REAL, DIMENSION(36)  :: LOCTH
      INTEGER R, RUNS
      COMMON IX, ANUMBE
      REAL ENERG, ANGL, PHI,X,Y,Z,OLDXY, NEWXY
      IX=35137
      ANUMBE=12
      RUNS=5000
      ...
      OPEN(UNIT=5,FILE='out.ENERG',STATUS='UNKNOWN')
      DO R=1,50
...
C *** Rods are set at a constant radius R away, spread evenly from 0 to 2*pi radians
        DO I=1,6
            LOCTH(I)=((I-1)/5.)*2*3.1415
        END DO
        DO I=7,18
            LOCTH(I)=((I-7.)/12)*2*3.1415
            END DO
        DO I=19,36
            LOCTH(I)=((I-19./24)*2*3.1415)
        END DO
        DO L=1,RUNS
            N=0
            CALL INITIAL(ENERG,PHI,ANGL,EX,EY,EZ,REAL(R),X,Y,Z)
...
C *** The R distance away that the rods are is based on only X and Y coordinates, so I found the the new and old R distances away using only X and Y.  Sign finds if it is traveling forwards or backwards.
                OLDXY=(X**2+Y**2)**(1./2)
                X=X+DIST*EX
                Y=Y+DIST*EY
                Z=Z+DIST*EZ
                NEWXY=(X**2+Y**2)**(1./2)
                IF (OLDXY.lt.NEWXY) THEN
                    SIGN=1
                ELSE
                    SIGN=-1
                END IF
                K=INROD(ENERG,REAL(R),OLDXY,NEWXY,ANGL,SIGN,LOCTH)
C *** 1 is Transmitted and thermalized, 2 is transmitted and not thermalized, 3 is backscattered and thermalized, and 4 is backscattered and not thermalized.
                IF(K.Eq.1)THEN
